const zhCN = {
  common: {
    noData: "暂无数据",
    user: "用户",
    topic: "话题",
    confirm: "确定",
    cancel: "取消",
    message: "消息",
    notLoggedIn: "请登录后查看此页面。",
    moreActions: "更多操作",
    pagination: {
      previous: "上一页",
      next: "下一页",
    },
    loadMore: {
      loadMore: "查看更多",
      noMore: "到底啦",
      loading: "加载中",
      error: "加载更多失败",
    },
    expand: "展开",
    collapse: "折叠",
    createBtn: {
      create: "发表",
      topic: "发帖",
      qa: "提问",
    },
    header: {
      profile: "个人中心",
      favorites: "我的收藏",
      editProfile: "编辑资料",
      dashboard: "管理后台",
      logout: "退出登录",
      login: "登录",
      confirmLogout: "确定退出登录吗？",
      messages: "消息",
      assigned: "我认领的",
      toggleMenu: "展开菜单",
    },
    footer: {
      poweredBy: "技术支持",
    },
    theme: {
      toggle: "切换主题",
      system: "跟随系统",
      light: "浅色模式",
      dark: "深色模式",
    },
    accessibility: {
      breadcrumb: "面包屑导航",
      close: "关闭",
      commandPalette: "命令面板",
      commandDescription: "搜索要执行的命令……",
      more: "更多",
      previousSlide: "上一张幻灯片",
      nextSlide: "下一张幻灯片",
      pagination: "分页",
      previousPage: "上一页",
      nextPage: "下一页",
      sidebar: "侧边栏",
      mobileSidebarDescription: "移动端侧边栏。",
      toggleSidebar: "切换侧边栏",
      loading: "加载中",
      slashCommands: "斜杠菜单",
      clearSelection: "清除选择",
      morePages: "更多页",
      mainNavigation: "主导航",
      mobileNavigation: "移动端导航",
      colorMode: "切换颜色模式",
      views: "浏览量",
      like: "点赞",
      comment: "评论",
      favorite: "收藏",
      backToTop: "返回顶部",
    },
  },
  dialog: {
    ok: "确定",
    cancel: "取消",
  },
  layout: {
    profile: {
      profile: "个人资料",
      accountSettings: "账号设置",
    },
  },
  dashboard: {
    brand: {
      name: "BBS-GO",
      plan: "管理后台",
      switcher: "工作区",
    },
    breadcrumb: {
      section: "控制台",
      overview: "概览",
    },
    header: {
      theme: "主题",
      language: "语言",
      siteHome: "打开前台首页",
    },
    theme: {
      system: "跟随系统",
      light: "浅色",
      dark: "深色",
    },
    nav: {
      platform: "平台",
      overview: "概览",
      workspace: "工作台",
      content: "内容",
      topics: "话题",
      categories: "分类",
      links: "链接",
      comments: "评论",
      tags: "标签",
      votes: "投票",
      voteOptions: "投票选项",
      community: "社区",
      users: "用户",
      userList: "用户列表",
      roles: "角色",
      userReports: "用户举报",
      favorites: "收藏",
      system: "系统",
      settings: "设置",
      siteSettings: "设置",
      operateLogs: "操作日志",
      dictionaries: "字典",
      dictionaryTypes: "字典类型",
      dictionaryItems: "字典项",
      voteRecords: "投票记录",
      attachments: "附件",
      messages: "站内消息",
      health: "健康与任务",
      trash: "回收站",
    },
    shortcuts: {
      title: "管理",
      home: "站点首页",
      categories: "分类",
      links: "链接",
      permissions: "权限",
      pages: "页面",
      more: "更多",
    },
    actions: {
      open: "打开",
      visit: "访问",
      remove: "移除",
      search: "搜索",
      sticky: "置顶",
      unsticky: "取消置顶",
      refresh: "刷新",
      create: "新建",
      edit: "编辑",
      delete: "删除",
      save: "保存",
      saveView: "保存视图",
      saveViewPrompt: "请输入视图名称",
      loadView: "加载已保存视图",
      title: "操作",
      view: "查看",
      resetPassword: "重置密码",
      recommend: "推荐",
      unrecommend: "取消推荐",
      undelete: "恢复",
      markSolved: "已解决",
      markUnsolved: "未解决",
      moveUp: "上移",
      moveDown: "下移",
      expand: "展开",
      collapse: "折叠",
      assignPermissions: "分配权限",
      viewAnswers: "查看回答",
      acceptAnswer: "采纳回答",
      unacceptAnswer: "取消采纳",
    },
    messages: {
      saved: "已保存",
      deleted: "已删除",
      actionDone: "操作完成",
      recommended: "已推荐",
      unrecommended: "已取消推荐",
      restored: "已恢复",
      markedSolved: "已标记为已解决",
      markedUnsolved: "已标记为未解决",
      stickied: "已置顶",
      unstickied: "已取消置顶",
      resetPassword: "新密码：{password}",
      reportProcessed: "举报已处理",
      reportIgnored: "举报已忽略",
      answerAccepted: "已采纳回答",
      answerUnaccepted: "已取消采纳",
      attachmentCleanupStarted: "孤儿附件清理任务已启动",
      messageTaskCreated: "群发草稿已创建",
      messageTaskStarted: "群发任务已启动",
      messageTaskRetried: "失败投递已重新启动",
    },
    confirm: {
      title: "确认操作",
      ok: "确认",
    },
    bulk: {
      title: "批量操作",
      selected: "已选择 {count} 条",
      selectAll: "选择当前可见记录",
      selectRow: "选择记录",
      clearSelection: "清除选择",
      scope: "继续前请检查 {count} 条已选记录的当前可操作范围。",
      eligible: "其中 {count} 条符合操作条件。",
      ineligible: "其中 {count} 条将被跳过。",
      result: "本次完成 {processed} 条，跳过 {skipped} 条，失败 {failed} 条。",
      failures: "失败记录：{count} 条",
      confirmationLabel: "请准确输入确认文本",
      confirm: "执行批量操作",
      retryFailures: "重试失败项",
    },
    resetPassword: {
      title: "密码重置成功",
      newPassword: "新密码",
      copy: "复制",
      copied: "已复制",
    },
    boolean: {
      yes: "是",
      no: "否",
    },
    forbidden: {
      ban7Days: "禁言7天",
      banForever: "永久禁言",
      remove: "解除禁言",
      batchBan7Days: "批量禁言7天",
      batchBanForever: "批量永久禁言",
      batchRemove: "批量解除禁言",
      permanent: "永久",
      reasonTemporary: "后台临时处置",
      reasonPermanent: "后台永久处置",
      confirm7Days: "确认禁言该用户7天？",
      confirmForever: "确认永久禁言该用户？",
      confirmRemove: "确认解除该用户禁言？",
      banned: "禁言状态已更新",
      removed: "已解除禁言",
      batchBanned: "已更新所选用户禁言状态",
      batchRemoved: "已解除所选用户禁言",
    },
    userAccess: {
      all: "全部节点",
      assignedCategories: "指定节点及子节点",
    },
    reportDataTypes: {
      topic: "帖子",
      comment: "评论",
      user: "用户",
    },
    reportProcessStatus: {
      pending: "待处理",
      processed: "已处理",
      ignored: "已忽略",
      unknown: "未知状态 {status}",
    },
    reportActions: {
      process: "处理",
      markProcessed: "标记为已处理",
      markIgnored: "标记为忽略",
      batchProcess: "批量标记处理",
      batchIgnore: "批量标记忽略",
      openTarget: "进入内容详情",
    },
    reportConfirm: {
      process: "确认将该举报标记为已处理？",
      ignore: "确认忽略该举报？",
    },
    reportProcess: {
      title: "处理举报",
      targetUnavailable: "被举报内容不存在或已被删除。",
      scope: "处理范围",
      scopeCurrent: "仅处理当前举报",
      scopeObject: "处理对象及其他 {count} 条待处理举报",
    },
    messageTask: {
      title: "群发消息",
      description: "先预览收件人数，再保存草稿并从任务记录启动发送。",
      targetAll: "全站用户",
      targetRole: "角色用户",
      targetCategory: "分类活跃用户",
      targetId: "角色或分类 ID",
      preview: "预览收件人",
      saveDraft: "保存草稿",
      recipientCount: "收件人 {count} 人",
      records: "发送记录",
      empty: "暂无群发任务",
      send: "发送",
      retry: "重试失败项",
    },
    answerDialog: {
      title: "回答管理",
      empty: "该话题暂无可用回答。",
    },
    gender: {
      male: "男",
      female: "女",
    },
    topicFeed: {
      untitled: "未命名话题",
      typeTopic: "话题",
      typeQa: "问答",
      statusNormal: "正常",
      statusDeleted: "已删除",
      qaSolved: "已解决",
      qaUnsolved: "未解决",
      voteMeta: "{optionCount}选{voteNum}",
      voteParticipants: "{count} 人参与",
      voteExpired: "投票已结束",
      voteExpiredAt: "截止时间：{time}",
    },
    topicBulk: {
      recommend: "批量推荐",
      delete: "批量删除",
      restore: "批量恢复",
    },
    voteTypes: {
      single: "单选",
      multiple: "多选",
    },
    status: {
      normal: "正常",
      deleted: "已删除",
      disabled: "已禁用",
    },
    period: {
      once: "一次性",
      daily: "每日",
    },
    categoryAttachment: {
      inherit: "继承上级或默认配置",
      custom: "使用本节点配置",
      enabled: "允许上传附件",
      disabled: "禁止上传附件",
    },
    roleTypes: {
      system: "系统",
      normal: "自定义",
    },
    rolePermissions: {
      title: "角色权限",
      description: "为 {role} 分配权限",
      empty: "暂无可分配权限",
    },
    permissionGroups: {
      workspace: "工作台",
      content: "内容管理",
      community: "社区用户",
      system: "系统管理",
      logs: "日志审计",
    },
    loading: "加载中...",
    confirmDelete: "确认删除这条记录吗？",
    confirmResetPassword: "确认重置该用户密码吗？",
    errors: {
      loadFailed: "加载失败",
      saveFailed: "保存失败",
      deleteFailed: "删除失败",
      actionFailed: "操作失败",
      sameLevelSortOnly: "树形记录只能在同一层级内排序。",
      invalidJson: "JSON 格式无效。",
      fixJsonBeforeSave: "请先修复无效的 JSON 字段再保存。",
      required: "必填项",
      invalidUrl: "请输入有效的 http(s) URL",
      invalidNumber: "请输入有效数字",
      minValue: "不能小于 {min}",
      maxValue: "不能大于 {max}",
    },
    pagination: {
      total: "共 {total} 条",
      previous: "上一页",
      next: "下一页",
      jump: "跳转",
    },
    settings: {
      common: "通用",
      content: "内容",
      spam: "反垃圾",
      notification: "通知",
      login: "登录",
      upload: "上传",
      page: "页面",
      script: "脚本",
      json: "JSON",
      siteTitle: "站点标题",
      siteLogo: "站点 Logo",
      siteDescription: "站点描述",
      baseURL: "站点 URL",
      siteKeywords: "站点关键词",
      siteNotification: "站点公告",
      tokenExpireDays: "Token 有效天数",
      moduleTopic: "话题模块",
      defaultCategoryId: "默认分类 ID",
      topicListStyle: "帖子列表样式",
      recommendTags: "推荐标签",
      urlRedirect: "链接跳转",
      enableHideContent: "回复可见",
      attachmentEnabled: "附件上传",
      attachmentAllowedTypes: "允许附件类型",
      attachmentMaxSizeMB: "附件大小 MB",
      attachmentMaxCount: "附件数量上限",
      topicCaptcha: "发帖验证码",
      userObserveSeconds: "新用户观察期秒数",
      notificationTypes: "通知类型",
      passwordLogin: "密码登录",
      weixinLogin: "微信登录",
      weixinAppId: "微信 App ID",
      weixinAppSecret: "微信 App Secret",
      googleLogin: "Google 登录",
      googleClientId: "Google Client ID",
      googleClientSecret: "Google Client Secret",
      githubLogin: "GitHub 登录",
      githubClientId: "GitHub Client ID",
      githubClientSecret: "GitHub Client Secret",
      enableUploadMethod: "上传方式",
      uploadLocal: "本地",
      uploadAliyunOss: "阿里云 OSS",
      uploadTencentCos: "腾讯云 COS",
      uploadAwsS3: "AWS S3",
      aliyunOssHost: "阿里云 OSS Host",
      aliyunOssBucket: "阿里云 OSS Bucket",
      aliyunOssEndpoint: "阿里云 OSS Endpoint",
      tencentCosBucket: "腾讯云 COS Bucket",
      tencentCosRegion: "腾讯云 COS Region",
      tencentCosSecretId: "腾讯云 SecretId",
      tencentCosSecretKey: "腾讯云 SecretKey",
      awsS3Region: "AWS S3 Region",
      awsS3Bucket: "AWS S3 Bucket",
      awsAccessKeyId: "AWS AccessKeyId",
      awsAccessKeySecret: "AWS AccessKeySecret",
      siteNavs: "站点导航",
      footerLinks: "底部链接",
      aboutPageConfig: "关于页面",
      scriptInjections: "脚本注入",
    },
    settingsForm: {
      common: {
        title: "通用配置",
        siteTitle: "网站名称",
        siteLogo: "网站LOGO",
        siteDescription: "网站描述",
        baseURL: "网站URL",
        siteKeywords: "网站关键字",
        siteNotification: "网站公告",
        submit: "提交",
        placeholder: {
          siteTitle: "网站名称",
          siteDescription: "网站描述",
          baseURL: "网站URL，例如：https://bbs.example.com",
          siteKeywords: "网站关键字",
          siteNotification: "网站公告（支持输入HTML）",
        },
      },
      announcement: {
        title: "站点公告发布",
        preview: "预览公告",
        previewResult: "预览结果",
        publish: "发布公告",
        published: "公告已发布",
        confirmTitle: "确认发布公告",
        confirmDescription: "发布后前台会立即显示新的公告内容，并写入发布记录。",
        history: "发布记录",
        empty: "暂无公告发布记录",
      },
      content: {
        title: "内容配置",
        sectionGeneral: "通用内容",
        sectionAttachment: "帖子附件",
        attachmentDefault: "默认附件限制",
        modules: "功能模块",
        topic: "帖子",
        qa: "提问",
        defaultCategoryId: "默认分类",
        topicListStyle: "帖子列表样式",
        topicListStyleDefault: "标准模式",
        topicListStyleCompact: "紧凑模式",
        recommendTags: "推荐标签",
        urlRedirect: "站外链接跳转页面",
        urlRedirectTooltip: "在跳转前需手动确认是否前往该站外链接",
        enableHideContent: "启用评论可见内容",
        enableHideContentTooltip: "发帖时支持设置评论后可见内容",
        attachmentEnabled: "开启附件上传",
        attachmentEnabledTooltip: "开启后发帖时可上传附件",
        attachmentAllowedTypes: "允许的附件类型",
        attachmentMaxSizeMB: "单个附件大小限制(MB)",
        attachmentMaxSizeMBTooltip:
          "单个附件最大体积，单位 MB；输入 0 表示不限制",
        attachmentMaxCount: "每篇帖子最多附件数",
        attachmentMaxCountTooltip:
          "每篇帖子最多可上传的附件数量；输入 0 表示不限制",
        placeholder: {
          defaultCategoryId: "发帖默认分类",
          recommendTags: "推荐标签",
          attachmentAllowedTypes:
            "如 .pdf .doc，可多选或输入新增；输入 * 表示允许任意扩展名",
        },
      },
      nav: {
        title: "导航配置",
        primaryList: "一级菜单列表",
        detailTitle: "菜单详情",
        untitled: "未命名菜单",
        groupMenu: "分组菜单（无链接）",
        operation: "操作",
        sort: "排序",
        noSelection: "请先在左侧选择一个一级菜单",
        tableTitle: "标题",
        tableUrl: "链接",
        tableOpenInNewWindow: "新窗口打开",
        childrenTitle: "二级菜单",
        childrenEmpty: "暂无二级菜单",
        groupUrlPlaceholder: "可选，作为分组菜单时可留空",
        empty: "暂无导航，请先新增一级菜单",
        submit: "提交",
        message: {
          validationTitleRequired: "请检查导航配置，标题不能为空",
          validationPrimaryUrlRequired: "一级菜单无子菜单时，链接不能为空",
          validationChildRequired: "请检查二级菜单配置，标题和链接不能为空",
        },
      },
      notification: {
        title: "通知配置",
        typesTitle: "消息类型开关",
        typeName: "消息类型",
        siteColumn: "站内信",
        types: {
          topicComment: "话题评论",
          commentReply: "评论回复",
          topicLike: "话题点赞",
          topicFavorite: "话题收藏",
          topicRecommend: "话题推荐",
          topicDelete: "话题被删除",
          qaAnswerAccepted: "采纳回答",
        },
        submit: "提交",
      },
      spam: {
        title: "反作弊配置",
        topicCaptcha: "发帖验证码",
        topicCaptchaTooltip: "发帖时是否开启验证码校验",
        userObserveSeconds: "用户观察期(秒)",
        userObserveSecondsTooltip:
          "观察期内用户无法发帖或提问，设置为 0 表示无观察期。",
        submit: "提交",
      },
      login: {
        title: "登录配置",
        weixinLogin: "微信登录",
        passwordLogin: "密码登录",
        googleLogin: "Google 登录",
        githubLogin: "GitHub 登录",
        enabled: "是否启用",
        appId: "AppID",
        appSecret: "AppSecret",
        clientId: "Client ID",
        clientSecret: "Client Secret",
        submit: "提交",
      },
      upload: {
        title: "上传配置",
        uploadConfig: "上传配置",
        enableUploadMethod: "启用上传方式",
        local: "本地存储",
        localTip:
          "文件将上传到服务端本地目录：res/uploads，并通过 /res/uploads/... 访问。",
        aliyunOss: "阿里云OSS",
        tencentCos: "腾讯云COS",
        awsS3: "AWS S3",
        host: "域名",
        bucket: "Bucket",
        endpoint: "Endpoint",
        accessKeyId: "AccessKey ID",
        accessKeySecret: "AccessKey Secret",
        region: "地域",
        secretId: "SecretId",
        secretKey: "SecretKey",
        imageStyleConfig: "图片样式配置",
        styleSplitter: "样式分隔符",
        styleAvatar: "头像样式",
        stylePreview: "预览样式",
        styleSmall: "小图样式",
        styleDetail: "详情样式",
        submit: "提交",
        placeholder: {
          host: "请输入OSS域名，如：https://xxx.oss-cn-beijing.aliyuncs.com/",
          bucket: "请输入Bucket名称",
          endpoint: "请输入Endpoint，如：oss-cn-beijing.aliyuncs.com",
          accessKeyId: "请输入AccessKey ID",
          accessKeySecret: "请输入AccessKey Secret",
          tencentBucket: "请输入Bucket名称，格式：bucket-appid",
          region: "请输入地域，如：ap-beijing",
          secretId: "请输入SecretId",
          secretKey: "请输入SecretKey",
          styleSplitter: "样式分隔符，如：!",
          styleAvatar: "头像样式，如：100x100",
          stylePreview: "预览样式，如：400x400",
          styleSmall: "小图样式，如：200x200",
          styleDetail: "详情样式，如：800x800",
          awsRegion: "请输入AWS区域，如：us-east-1",
          awsBucket: "请输入Bucket名称",
          awsAccessKeyId: "请输入AccessKey ID",
          awsSecretAccessKey: "请输入Secret Access Key",
        },
      },
      script: {
        title: "脚本注入",
        tip: "脚本会全站注入到 site 的 head 中，请仅配置可信代码。",
        add: "新增脚本",
        remove: "删除",
        enabled: "是否启用",
        type: "脚本类型",
        external: "外链脚本",
        inline: "内联脚本",
        src: "脚本地址",
        async: "异步加载",
        defer: "延迟执行",
        crossorigin: "跨域属性",
        code: "脚本代码",
        submit: "提交",
        placeholder: {
          scriptName: "脚本名称",
          src: "https://example.com/script.js",
          crossorigin: "anonymous",
          code: "window.dataLayer = window.dataLayer || [];",
        },
        message: { scriptNameRequired: "脚本名称不能为空" },
      },
      page: {
        title: "页面配置",
        aboutTitle: "关于页内容",
        aboutHelp: "关于页内容支持 Markdown，建议同时维护中英文内容。",
        footerLinksTitle: "底部链接与信息",
        addFooterLink: "新增底部项",
        remove: "删除",
        visible: "显示",
        openInNewWindow: "新窗口打开",
        zhCNContent: "中文内容",
        enUSContent: "英文内容",
        zhCNText: "中文文本",
        enUSText: "英文文本",
        urlOptional: "链接（可选）",
        footerLinksEmpty: "暂无底部项",
        footerLinkItem: "底部项 #{index}",
        placeholder: {
          aboutContent: "请输入关于页 Markdown 内容",
          linkText: "如：关于、友情链接、ICP备案号、公安网备信息",
          linkUrl: "可留空；支持站内相对路径或完整 URL",
        },
        message: {
          footerLinkInvalid: "请检查底部项，文本不能为空",
          footerLinkUrlInvalid:
            "请检查底部项链接，仅支持站内相对路径、锚点或 http(s) URL",
        },
      },
      search: {
        title: "搜索索引",
        description:
          "当搜索结果缺失、过期，或批量调整内容后，可以手动重建帖子、问答、用户索引。",
        indexedContent: "索引内容",
        indexedContentTopics: "帖子",
        indexedContentAll: "帖子、问答、用户",
        status: "状态",
        statusIdle: "未开始",
        statusRunning: "重建中",
        statusFinished: "已完成",
        statusFailed: "失败",
        progressText: "{processed} / {total}（{percent}%）",
        topicProgress: "帖子 {processed} / {total}",
        userProgress: "用户 {processed} / {total}",
        operation: "操作",
        rebuild: "重建索引",
        rebuilding: "重建中",
        confirmTitle: "确认重建搜索索引？",
        confirmDescription:
          "系统会在后台扫描现有帖子、问答、用户并重建搜索索引。重建期间搜索结果可能短暂不完整。",
        confirmAction: "开始重建",
        started: "搜索索引重建已开始。",
      },
      sitemap: {
        title: "站点地图",
        description:
          "当内容、分类、标签或站点地址调整后，可以手动重新生成 sitemap XML 文件和索引。",
        content: "Sitemap 内容",
        contentAll: "静态页面、话题分类、标签、帖子、问答",
        status: "状态",
        statusIdle: "未开始",
        statusRunning: "生成中",
        statusFinished: "已完成",
        statusFailed: "失败",
        operation: "操作",
        generate: "生成 Sitemap",
        generating: "生成中",
        publicURL: "公开地址",
        open: "打开 sitemap",
        confirmTitle: "确认生成 Sitemap？",
        confirmDescription:
          "系统会在后台重新生成 sitemap 文件，上传成功后 /sitemap.xml 会指向新的 sitemap 索引。",
        confirmAction: "开始生成",
        started: "Sitemap 生成已开始。",
      },
    },
    fields: {
      id: "ID",
      idEncode: "公开ID",
      username: "用户名",
      nickname: "昵称",
      password: "密码",
      type: "类型",
      gender: "性别",
      avatar: "头像",
      roles: "角色",
      permissions: "权限",
      homePage: "主页",
      description: "描述",
      status: "状态",
      processStatus: "处理状态",
      createTime: "创建时间",
      updateTime: "更新时间",
      title: "标题",
      topic: "帖子",
      user: "用户",
      userId: "用户ID",
      recommend: "推荐",
      qaStatus: "问答状态",
      vote: "投票",
      viewCount: "浏览",
      commentCount: "评论",
      cover: "封面",
      node: "分类",
      category: "分类",
      name: "名称",
      parentId: "父级分类",
      logo: "图标",
      sortNo: "排序",
      attachmentPolicyConfigured: "附件限制配置",
      attachmentEnabled: "附件上传",
      attachmentAllowedTypes: "允许的附件类型",
      attachmentMaxSizeMB: "单个附件大小(MB)",
      attachmentMaxCount: "每帖附件数量",
      url: "URL",
      word: "词条",
      remark: "备注",
      icon: "图标",
      summary: "摘要",
      forbidden: "禁言",
      forbiddenEndTime: "禁言结束时间",
      contentAccessMode: "内容访问级别",
      categoryIds: "可访问节点",
      bizType: "业务类型",
      subject: "主题",
      errorMsg: "错误",
      content: "内容",
      method: "方法",
      path: "路径",
      component: "组件",
      code: "编码",
      eventCount: "事件次数",
      maxFinishCount: "最多完成",
      btnName: "按钮",
      actionUrl: "跳转地址",
      dataType: "数据类型",
      dataId: "数据ID",
      entityType: "内容类型",
      entityId: "内容ID",
      reason: "原因",
      reportTarget: "举报对象",
      ip: "IP地址",
      userAgent: "User-Agent",
      referer: "来源页",
      opType: "操作类型",
      dictType: "字典类型",
      label: "标签文本",
      value: "值",
      topicId: "话题ID",
      voteId: "投票ID",
      voteType: "投票类型",
      voteNum: "最多可选数量",
      optionCount: "选项数量",
      voteCount: "投票数",
      expiredAt: "截止时间",
      optionIds: "选项ID",
      quoteId: "引用评论ID",
      parentComment: "父级评论",
      fileName: "文件名",
      fileUrl: "存储地址",
      fileType: "文件类型",
      fileSize: "文件大小",
      downloadCount: "下载次数",
      fromId: "发送人ID",
      unread: "未读",
      read: "已读",
      active: "正常",
      deleted: "已删除",
    },
    pages: {
      users: {
        title: "用户管理",
        description: "搜索、查看、编辑用户账号并重置密码。",
      },
      topics: {
        title: "话题管理",
        description: "管理话题并执行删除等操作。",
      },
      categories: {
        title: "分类管理",
        description: "管理话题和问答分类。",
      },
      links: {
        title: "友情链接",
        description: "管理站点展示的外部链接。",
      },
      comments: {
        title: "评论管理",
        description: "查看授权话题中的评论并删除违规内容。",
      },
      userReports: {
        title: "用户举报",
        description: "查看社区用户提交的内容举报。",
      },
      roles: {
        title: "角色管理",
        description: "管理角色，并分配后台权限。",
      },
      settings: {
        title: "系统设置",
        description: "编辑后台配置载荷，后续可继续拆分为分组表单。",
      },
      operateLogs: {
        title: "操作日志",
        description: "追踪后台操作、操作来源和相关对象。",
      },
      tags: {
        title: "标签管理",
        description: "创建、编辑和启用或停用话题标签。",
      },
      dictionaryTypes: {
        title: "字典类型",
        description: "管理可复用的字典分类和编码。",
      },
      dictionaryItems: {
        title: "字典项",
        description: "管理层级字典值、状态和排序。",
      },
      favorites: {
        title: "收藏数据",
        description: "只读查看用户收藏，不修改用户行为记录。",
      },
      votes: {
        title: "投票管理",
        description: "管理投票定义并查看参与统计。",
      },
      voteOptions: {
        title: "投票选项",
        description: "查看和纠正投票所属选项。",
      },
      voteRecords: {
        title: "投票记录",
        description: "只读查看投票记录，不修改用户行为。",
      },
      attachments: {
        title: "附件管理",
        description: "按文件、类型、用户和帖子查询附件，删除前保留引用记录。",
        cleanupTitle: "孤儿附件清理",
        cleanupDescription: "异步删除超过 7 天且尚未绑定帖子的附件对象。",
      },
      messages: {
        title: "站内消息",
        description: "向单个用户发送可追踪的站内消息，并查看发送记录。",
      },
      health: {
        title: "健康与后台任务",
        description: "查看数据库、存储和后台任务状态，按权限启动受控任务。",
        components: "服务组件",
        tasks: "后台任务",
        taskFailed: "存在失败任务",
        taskRunning: "任务运行中",
        taskIdle: "暂无运行任务",
        taskReady: "暂无错误",
        start: "启动",
        componentNames: {
          database: "数据库",
          storage: "存储",
          search: "搜索",
          sitemap: "Sitemap",
        },
        taskNames: {
          searchReindex: "搜索索引重建",
          sitemap: "Sitemap 生成",
          databaseBackup: "数据库备份",
        },
        actionFailed: "操作失败",
        backup: {
          title: "数据库备份",
          noErrors: "暂无错误",
          enabled: "启用自动备份（重启后生效）",
          progressTitle: "备份进度",
          restoreProgressTitle: "回档进度",
          progressPhase: "当前阶段",
          phaseWorking: "处理中",
          phasePreparing: "准备中",
          phaseDumping: "导出数据库",
          phaseVerifying: "校验备份文件",
          phaseSafetyBackup: "创建当前数据库安全备份",
          phaseRestoring: "导入数据库",
          schedule: "Cron 计划",
          scheduleHelpLabel: "查看 Cron 计划说明",
          scheduleHelp: "格式为五段：分钟、小时、日期、月份、星期。\n0 3 * * * = 每天 03:00\n*/15 * * * * = 每 15 分钟\n0 2 * * 0 = 每周日 02:00",
          retention: "保留份数",
          directory: "备份目录",
          directoryHelpLabel: "查看备份目录说明",
          directoryHelp: "请勿轻易修改备份目录。此处填写的是容器内相对路径，不能填写绝对路径。\n在项目提供的 Docker Compose 配置中，保持默认值 backups 时，备份文件会通过挂载保存到 Docker 外部：本地开发为 ./docker-data/backups，部署包为 ./data/backups（实例部署为 ./runtime/backups）。\n如果改成其他目录，必须同步修改 Docker Compose 的宿主机挂载，否则备份可能只保存在容器内，删除或重建容器后会丢失。",
          file: "文件",
          status: "状态",
          size: "大小（字节）",
          finishedAt: "完成时间",
          actions: "操作",
          success: "成功",
          running: "执行中",
          failed: "失败",
          empty: "暂无备份记录",
          create: "立即备份",
          created: "备份任务已启动",
          saved: "备份配置已保存",
          deleted: "备份已删除",
          restore: "回档",
          restoreStarted: "回档任务已启动",
          restoreRunning: "数据库回档执行中",
          createTitle: "创建数据库备份",
          createDescription: "备份可能占用较多磁盘和数据库资源，确认立即创建吗？",
          saveTitle: "保存备份配置",
          saveDescription: "自动备份计划和保留策略会写入服务配置，确认保存吗？",
          download: "下载",
          downloadTitle: "下载数据库备份",
          downloadDescription: "备份文件可能包含完整站点数据，请确认下载并妥善保管。",
          delete: "删除",
          deleteTitle: "删除数据库备份",
          deleteDescription: "删除后无法从后台恢复该备份文件，确认继续吗？",
          restoreTitle: "回档数据库备份",
          restoreDescription: "系统会先创建当前数据库的安全备份，再导入该 MySQL 备份；现有数据可能被覆盖，确认继续吗？",
          orphans: {
            title: "发现文件",
            empty: "备份目录中未发现未注册的备份文件。",
            scan: "扫描目录",
            scanning: "扫描中…",
            adopt: "注册",
            adopted: "文件已注册",
            valid: "有效",
            invalid: "无效",
            registered: "已注册",
            adoptTitle: "注册备份文件",
            adoptDescription: "该文件将加入备份列表，之后可通过后台下载、回档或删除。确认继续吗？",
            type: "类型",
          },
        },
      },
      trash: {
        title: "回收站",
        description: "查看和恢复已软删除的帖子、评论和附件。",
        tabTopics: "已删帖子",
        tabComments: "已删评论",
        tabAttachments: "已删附件",
        empty: "没有已删除的内容。",
        restored: "「{label}」已恢复。",
        restoreConfirmTitle: "恢复内容",
        restoreConfirmDesc: "确认恢复「{label}」到正常显示状态？",
      },
    },
    user: {
      anonymous: "访客",
      account: "账号",
      notifications: "通知",
      changePassword: "修改密码",
      passwordChange: {
        title: "修改管理员密码",
        currentPassword: "当前密码",
        newPassword: "新密码",
        confirmPassword: "确认新密码",
        submit: "暂存密码修改",
        submitting: "暂存中...",
        mismatch: "两次输入的新密码不一致。",
        formDescription: "新密码不会立即生效。提交后请在新窗口中使用新密码登录以完成确认。",
        pendingTitle: "新密码已暂存",
        pendingNotice: "当前密码仍然有效。请在 10 分钟内用新密码在新窗口登录完成确认。",
        pendingStep1: "点击下方「打开登录窗口」",
        pendingStep2: "在新窗口中使用新密码登录",
        pendingStep3: "登录成功后密码正式切换，当前会话自动退出",
        pendingDescription:
          "新密码已暂存，请在 10 分钟内打开新的登录窗口并使用新密码登录。新密码登录成功前，当前密码仍然有效。",
        openSignin: "打开登录窗口",
      },
      logout: "退出登录",
    },
    batchRegister: {
      title: "批量注册",
      description: "输入邮箱列表（分号分隔），系统将自动用邮箱前缀作为用户名、随机生成强密码完成注册。",
      emailsLabel: "邮箱列表",
      emailsPlaceholder: "xjhuang@tsntec.com;caihang@tsntec.com;chenbin@tsntec.com;",
      submit: "开始批量注册",
      submitting: "注册中...",
      resultTitle: "注册结果",
      copyAll: "复制全部",
      copied: "已复制到剪贴板",
      columns: { username: "用户名", email: "邮箱", password: "密码", status: "状态" },
      status: { created: "已创建", skipped: "已跳过", failed: "失败" },
      summary: "共 {total} 个，成功 {created}，跳过 {skipped}，失败 {failed}",
    },
    overview: {
      hero: {
        eyebrow: "社区运营工作台",
        title: "欢迎回来，{name}",
        description:
          "这里是 BBS-GO 管理后台，你可以在这里查看社区数据、处理举报，并快速进入常用管理页面。",
        openSite: "查看前台首页",
        manageContent: "管理内容",
      },
      metrics: {
        totalUsers: "用户总数",
        totalTopics: "话题总数",
        todayUsers: "今日新增用户",
        todayTopics: "今日新增话题",
        todayComments: "今日新增评论",
        activeUsers: "今日活跃用户",
        forbiddenUsers: "当前禁言用户",
        failedTasks: "失败后台任务",
        failedMessageTasks: "失败群发任务",
      },
      pending: {
        title: "待处理事项",
        description: "优先处理举报和异常，让社区保持健康运转。",
        pendingReports: "待处理举报",
        goHandle: "去处理",
      },
      priority: {
        title: "今日优先级",
        description: "根据你有权限处理的举报数量排序。",
        emptyTitle: "暂无待办",
        emptyDescription: "当前没有需要优先处理的举报或异常。",
      },
      quick: {
        title: "快捷入口",
        description: "只展示当前账号有权限访问的常用功能。",
        topics: "内容管理",
        users: "用户管理",
        categories: "分类管理",
        settings: "站点设置",
      },
      recent: {
        title: "最近活动",
        topics: "最近话题",
        users: "最近注册用户",
      },
      trend: {
        title: "近 7 日趋势",
        description: "按天查看用户、内容、评论和举报变化。",
        users: "用户",
        topics: "话题",
        comments: "评论",
        reports: "举报",
      },
      breakdown: {
        categories: "分类内容分布（话题 / 评论）",
        roles: "角色用户分布",
      },
    },
  },
  captcha: {
    title: "请拖动滑块将图片转正",
    code: "验证码",
    codePlaceholder: "请输入验证码",
      refresh: "刷新",
    loading: "验证码加载中",
    loadFailed: "验证码加载失败",
  },
  composables: {
    justNow: "刚刚",
    minutesAgo: "{n} 分钟前",
    hoursAgo: "{n} 小时前",
    daysAgo: "{n} 天前",
    pleaseSignIn: "请先登录",
    unknownError: "未知错误",
  },
  component: {
    searchInput: {
      placeholder: "搜索帖子、问答、用户",
      searchBtn: "搜索",
    },
    search: {
      allCategories: "全部",
      timeRange: {
        all: "时间不限",
        day: "一天内",
        week: "一周内",
        month: "一月内",
        year: "一年内",
      },
    },
    siteNotice: {
      title: "公告",
    },
    friendLinks: {
      title: "友情链接",
      more: "查看更多",
    },
    userInfo: {
      topicCount: "话题",
      commentCount: "评论",
    },
    myCounts: {
      title: "个人成就",
      topicCount: "话题",
      commentCount: "评论",
    },
    activitySummary: {
      topics: "发帖",
      comments: "回答",
      acceptedAns: "被采纳",
      claims: "认领中",
      claimed: "你认领了「{title}」",
      answered: "你回答了「{title}」",
      accepted: "你的回答被采纳：「{title}」",
      topicCreated: "你发布了「{title}」",
    },
    myProfile: {
      title: "个人资料",
      editProfile: "编辑资料",
      nickname: "昵称",
      description: "签名",
      homePage: "主页",
    },
    avatarEdit: {
      update: "点击修改",
      updateSuccess: "头像更新成功",
      updateFailed: "头像更新失败",
    },
    imageUpload: {
      uploading: "上传中...",
      upload: "上传图片",
      uploadFailed: "文件 {name} 上传失败: {error}",
      countLimitError: "最多只能上传 {limit} 张图片",
    },
    tagInput: {
      placeholder: "请输入标签",
      recommendTags: "推荐标签",
    },
    userReport: {
      title: "举报",
      description: "请选择举报原因，必要时补充说明。",
      placeholder: "补充说明（选填）",
      submit: "提交举报",
      submitting: "提交中...",
      success: "举报已提交",
      reason: {
        spam: "垃圾广告",
        illegal: "违法违规",
        harassment: "人身攻击",
        pornographic: "色情低俗",
        other: "其他",
      },
    },
    topicManageMenu: {
      edit: "修改",
      delete: "删除",
      recommend: "推荐",
      cancelRecommend: "取消推荐",
      sticky: "置顶",
      cancelSticky: "取消置顶",
      forbidden7Days: "禁言7天",
      forbiddenForever: "永久禁言",
      forbiddenSuccess: "禁言成功",
      forbiddenFailed: "禁言失败",
      confirmDelete: "是否确认删除该话题？",
      deleteSuccess: "删除成功",
      deleteFailed: "删除失败：",
      confirmAction: "是否确认{action}该话题？",
      actionSuccess: "{action}成功",
      actionFailed: "{action}失败：",
      report: "举报",
    },
    editorMode: {
      label: "编辑器",
      switchLabel: "切换编辑模式",
      visual: "可视化编辑",
      htmlSource: "HTML 源码",
      markdown: "Markdown",
      switchConfirm: "切换到{mode}会清空当前内容，是否继续？",
    },
    htmlEditor: {
      toolbar: {
        edit: "仅编辑",
        split: "分屏预览",
        preview: "仅预览",
        fullscreen: "全屏",
        exitFullscreen: "退出全屏",
      },
    },
    textEditor: {
      placeholder: "请输入您要发表的内容",
      publish: "发布",
      pleaseWait: "请先等待图片上传完成",
    },
    richTextEditor: {
      placeholder: "输入 / 插入内容",
      toolbar: {
        undo: "撤销",
        redo: "重做",
        bold: "粗体",
        underline: "下划线",
        italic: "斜体",
        strike: "删除线",
        heading1: "标题 1",
        heading2: "标题 2",
        quote: "引用",
        bulletList: "无序列表",
        orderedList: "有序列表",
        taskList: "任务列表",
        alignLeft: "左对齐",
        alignCenter: "居中对齐",
        alignRight: "右对齐",
        textColor: "文字颜色",
        backgroundColor: "背景颜色",
        clearColor: "清除颜色",
        inlineCode: "行内代码",
        codeBlock: "代码块",
        link: "链接",
        image: "插入图片",
        horizontalRule: "分割线",
        table: "插入表格",
        highlight: "高亮标记",
        fontSize: "字体大小",
        superscript: "上标",
        subscript: "下标",
        clearFormat: "清除格式",
        addRowBefore: "上方插入行",
        addRowAfter: "下方插入行",
        addColBefore: "左侧插入列",
        addColAfter: "右侧插入列",
        deleteRow: "删除行",
        deleteCol: "删除列",
        deleteTable: "删除表格",
        fullscreen: "全屏",
        exitFullscreen: "退出全屏",
        scrollForward: "向右滚动查看更多",
        scrollBack: "向左滚动",
        uploading: "上传中...",
      },
      linkDialog: {
        textLabel: "链接文本",
        urlLabel: "链接地址",
        textPlaceholder: "请输入链接显示文本",
        urlPlaceholder: "请输入链接地址",
        confirm: "应用",
        remove: "移除链接",
        cancel: "取消",
      },
      slash: {
        hintContinueTyping: "继续输入关键词筛选菜单...",
        noMatchingCommand: "没有找到匹配的命令",
        paragraph: {
          title: "正文",
          description: "普通段落文本",
        },
        heading1: {
          title: "标题 1",
          description: "大标题",
        },
        heading2: {
          title: "标题 2",
          description: "二级标题",
        },
        heading3: {
          title: "标题 3",
          description: "三级标题",
        },
        bulletList: {
          title: "无序列表",
          description: "创建无序列表",
        },
        orderedList: {
          title: "有序列表",
          description: "创建有序列表",
        },
        taskList: {
          title: "任务列表",
          description: "创建待办清单",
        },
        quote: {
          title: "引用",
          description: "插入引用文本",
        },
        codeBlock: {
          title: "代码块",
          description: "插入代码块",
        },
        horizontalRule: {
          title: "分割线",
          description: "插入水平分割线",
        },
        calloutInfo: {
          title: "信息面板",
          description: "插入信息提示框",
        },
        calloutWarning: {
          title: "警告面板",
          description: "插入警告提示框",
        },
        calloutTip: {
          title: "提示面板",
          description: "插入提示框",
        },
        calloutSuccess: {
          title: "成功面板",
          description: "插入成功提示框",
        },
      },
    },
    userProfile: {
      setBackground: "设置背景",
      backgroundSuccess: "背景设置成功",
    },
    userCenterSidebar: {
      operations: "操作",
      removeForbidden: "取消禁言",
      forbidden7Days: "禁言7天",
      forbiddenForever: "永久禁言",
      confirmForbidden: "是否禁言该用户？",
      confirmForbiddenForever: "是否永久禁言该用户？",
      dialogTitle: "提示",
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      forbiddenSuccess: "禁言成功",
      forbiddenFailed: "禁言失败",
      removeForbiddenSuccess: "取消禁言成功",
      removeForbiddenFailed: "取消禁言失败",
      report: "举报",
    },
    fansWidget: {
      title: "粉丝",
      more: "查看更多",
    },
    followWidget: {
      title: "关注",
      more: "查看更多",
    },
    followBtn: {
      follow: "关注",
      followed: "已关注",
    },
    setPasswordDialog: {
      title: "设置密码",
      passwordPlaceholder: "请输入密码",
      rePasswordPlaceholder: "请再次输入密码",
      success: "设置密码成功",
    },
    setUsernameDialog: {
      title: "设置用户名",
      usernamePlaceholder: "请输入用户名",
      usernameRule:
        "用户名只能设置一次，请谨慎操作。用户名必须由5-12位(数字、字母、_、-)组成，且必须以字母开头。",
      success: "用户名设置成功",
    },
    updatePasswordDialog: {
      title: "设置密码",
      oldPasswordPlaceholder: "请输入当前密码",
      newPasswordPlaceholder: "请输入新密码",
      rePasswordPlaceholder: "请再次输入新密码",
      success: "修改密码成功",
    },
    wxBindDialog: {
      title: "微信绑定",
      scanTip: "请扫码绑定微信",
      bindingText: "微信授权绑定中...",
    },
    googleBindDialog: {
      title: "绑定 Google 账号",
      bindTip: "点击下方按钮跳转到 Google 授权页面完成绑定",
      binding: "绑定中...",
      bindButton: "使用 Google 账号绑定",
      bindingText: "Google授权绑定中...",
    },
    githubBindDialog: {
      title: "绑定 GitHub 账号",
      bindTip: "点击下方按钮跳转到 GitHub 授权页面完成绑定",
      binding: "绑定中...",
      bindButton: "使用 GitHub 账号绑定",
      bindingText: "GitHub 授权绑定中...",
    },
    topicList: {
      sticky: "置顶",
      qaSolved: "已解决",
      qaUnsolved: "未解决",
      lastReply: "最后回复 {time}",
      hasImages: "含图片",
      imageMark: "图",
      imageMarkWithCount: "图 {count}",
      hasVote: "含投票",
      voteMark: "投票",
      likeSuccess: "点赞成功",
      unlikeSuccess: "取消点赞成功",
    },
    sideActionBar: {
      likeSuccess: "点赞成功",
      likeCancel: "已取消点赞",
      favoriteSuccess: "收藏成功",
      favoriteCancel: "已取消收藏",
    },
    comment: {
      title: "评论",
      loginLink: "请登录后发表观点",
      deleteConfirm: "确定删除这条评论吗？",
      deleteSuccess: "删除成功",
      input: {
        replyTo: "回复：",
        cancelReply: "取消回复",
        pleaseInput: "请输入评论内容",
        publishSuccess: "发布成功",
      },
      list: {
        acceptedAnswer: "已采纳",
        ipLocation: "IP属地",
        like: "点赞",
        liked: "已赞",
        reply: "评论",
        cancelReply: "取消评论",
        publishSuccess: "发布成功",
        acceptAnswer: "采纳回答",
        unacceptAnswer: "取消采纳",
        delete: "删除",
        report: "举报",
        acceptSuccess: "采纳成功",
        unacceptSuccess: "已取消采纳",
      },
      subList: {
        replyTo: "回复",
        like: "点赞",
        liked: "已赞",
        reply: "评论",
        cancelReply: "取消评论",
        delete: "删除",
        report: "举报",
        loadMore: "查看更多回复",
      },
    },
    claim: {
      title: "认领",
      claimIt: "认领此问题",
      unclaim: "取消认领",
      claimed: "你已认领此问题。",
      youClaimed: "您已认领此提问",
      cancel: "取消",
      you: "你",
      noClaims: "暂无认领者。",
      remove: "移除",
    },
    attachment: {
      count: "个文件",
    },
  },
  pages: {
    home: {
      asideTitle: "BBS-GO",
      asideDescription: "发现最新帖子、问答和社区内容。",
    },
    nav: {
      home: "首页",
      topics: "帖子",
      search: "搜索",
    },
    topics: {
      title: "话题",
      feedLatest: "最新",
      feedRecommend: "推荐",
      feedFollowing: "关注",
      filterLatestPublish: "最新发布",
      filterLatestReply: "最新回复",
      subCategories: "子分类",
      allCategories: "全部",
      moreCategories: "更多分类",
      moreSubCategories: "更多子分类",
      stats: {
        topics: "帖子",
        qa: "问答",
        solved: "已解决",
        unsolved: "未解决",
      },
    },
    topic: {
      categorySelector: {
        title: "选择分类",
        choose: "请选择分类",
        more: "更多",
        subcategories: "子分类",
        searchPlaceholder: "搜索分类",
        recent: "最近使用",
        empty: "没有匹配的分类",
        expand: "展开子分类",
        collapse: "收起子分类",
        mobileHint: "点这里查看全部分类",
      },
      detail: {
        publishedAt: "发布于",
        qaSolved: "已解决",
        qaUnsolved: "待解决",
        answers: "回答",
        view: "浏览",
        like: "点赞",
        likeSuccess: "点赞成功",
        comment: "评论",
        favorite: "收藏",
        favoriteSuccess: "收藏成功",
        tocTitle: "目录",
        ipLocation: "IP 属地：",
        hideContent: "隐藏内容",
        hideContentTip: "隐藏内容，回复后可见",
        comments: "评论",
        attachments: "附件",
        attachmentDownloadCount: "下载 {count} 次",
        download: "下载",
        vote: {
          tag: "投票",
          single: "单选",
          multiple: "多选（最多 {num} 项）",
          titleMeta: "{optionCount}选{voteNum}",
          expiredAt: "截止时间",
          total: "总票数",
          participants: "{count}人参与",
          selectedCount: "已选择 {selected}/{max}",
          expired: "投票已截止",
          voted: "你已投票",
          loginToVote: "登录后可参与投票",
          waiting: "请选择后提交投票",
          submit: "提交投票",
          voteEnded: "投票结束",
          expandOptions: "展开查看更多选项",
          collapseOptions: "收起选项",
          submitSuccess: "投票成功",
          maxSelect: "最多可选择 {num} 项",
          votedShort: "已投",
          selectRequired: "请选择一个选项",
          selectAtLeastOne: "请至少选择一个选项",
        },
      },
      create: {
        post: "发帖",
        qa: "提问",
        titlePlaceholder: "请输入帖子标题",
        contentPlaceholder: "请输入你要发表的内容...",
        postBtn: "发表帖子",
        qaBtn: "发布问题",
        switchEditorConfirm: "切换编辑器将会清空当前内容，是否继续？",
        imageUploading: "图片上传中,请稍后重试...",
        attachmentUploading: "附件上传中，请稍后再提交",
        noCategoryTitle: "暂无可用分类",
        noCategoryDescription:
          "当前没有该内容类型的可写分类，无法创建内容。",
        categoryRequiredTitle: "请先选择分类",
        categoryRequiredDescription:
          "选择一个可写分类后，才能填写标题、正文和其他发布内容。",
        categoryRequiredSubmit: "请先选择一个可写分类。",
        topicFeatureDisabled: "帖子模块未开启",
        qaFeatureDisabled: "提问模块未开启",
        draft: {
          saveBtn: "保存草稿",
          listBtn: "草稿列表",
          delete: "删除草稿",
          saved: "帖子草稿已保存",
          savedQa: "提问草稿已保存",
        },
        attachment: {
          label: "附件（可选）",
          limitHint: "最多 {maxCount} 个，单个 ≤ {maxSizeMB} MB",
          sizeUnlimitedHint: "最多 {maxCount} 个，单个大小不限制",
          countUnlimitedHint: "附件数量不限制，单个 ≤ {maxSizeMB} MB",
          unlimitedHint: "附件数量和大小均不限制",
          add: "添加附件",
          remove: "移除",
        },
        vote: {
          addTitle: "添加投票",
          editTitle: "编辑投票",
          addEntry: "添加投票",
          added: "已添加投票",
          edit: "编辑投票",
          remove: "移除投票",
          titlePlaceholder: "请输入投票标题",
          single: "单选",
          multiple: "多选（最多 {num} 项）",
          multipleShort: "多选",
          expiredAt: "截止时间",
          voteNum: "最多可选",
          addOption: "添加选项",
          optionPlaceholder: "选项 {index}",
          optionsCount: "个选项",
          validateTitle: "请输入投票标题",
          validateExpiredAt: "请设置未来的截止时间",
          validateOptionCount: "至少需要 2 个选项",
          validateOptionDuplicate: "选项内容不能重复",
          validateVoteNum: "最多可选数量不合法",
        },
      },
      edit: {
        title: "修改帖子",
        pageTitle: "修改话题",
        titlePlaceholder: "请输入帖子标题",
        contentPlaceholder: "请输入你要发表的内容...",
        noWritableCategoryTitle: "当前没有可编辑的分类",
        noWritableCategoryDescription:
          "你没有该类型帖子的可写分类权限，因此无法修改此内容。",
        submitBtn: "提交更改",
        switchEditorConfirm: "切换编辑器将会清空当前内容，是否继续？",
        success: "修改成功",
        failed: "提交失败：{msg}",
        attachmentItem: "附件 {n}",
      },
    },
    qa: {
      filterAll: "全部",
      filterUnsolved: "待解决",
      filterSolved: "已解决",
    },
    search: {
      title: "搜索",
      heading: "搜索内容",
      description: "快速查找社区里的帖子、问答和用户。",
      placeholder: "搜索帖子、问答、用户",
      submit: "搜索",
      empty: {
        title: "没有找到相关结果",
        description: "换个关键词，或调整内容类型和筛选条件再试试。",
        promptTitle: "开始搜索",
        promptDescription: "输入关键词后会展示帖子、问答和用户结果。",
      },
      viewMore: "查看更多",
      userTopics: "{count} 个帖子",
      userFans: "{count} 个粉丝",
      tabs: {
        all: "全部",
        topic: "帖子",
        user: "用户",
      },
      node: "分类",
      allCategories: "全部分类",
      timeRange: "时间范围",
      timeRangeAll: "全部",
      timeRangeDay: "1天",
      timeRangeWeek: "1周",
      timeRangeMonth: "1月",
      timeRangeYear: "1年",
    },
    about: {
      title: "关于我们",
    },
    links: {
      title: "友情链接",
    },
    redirect: {
      link: "点击该链接继续跳转 >>",
      error: "你访问的页面发生错误!",
    },
    error: {
      header: "页面出了点问题",
      notFound: "页面没找到",
      forbidden: "无权限",
      unknown: "异常",
      title404: "你访问的页面不见了",
      desc404: "链接可能已失效，或页面已被移动/删除。",
      title403: "你没有访问该页面的权限",
      desc403: "当前账号没有后台管理权限，请切换到管理员账号后再访问。",
      title500: "服务器开小差了",
      desc500: "请稍后重试。如果问题持续出现，请联系站点管理员。",
      backHome: "返回首页",
      goBack: "返回上一页",
      retry: "重新加载",
      searchPlaceholder: "搜索",
      quickHome: "去看最新讨论",
      quickTopics: "浏览全部主题",
    },
    install: {
      title: "BBS-GO 安装引导",
      step: {
        welcome: "欢迎使用",
        database: "数据库配置",
        site: "站点信息",
        admin: "管理员设置",
        install: "安装中",
        complete: "安装完成",
      },
      welcome: {
        description:
          "欢迎使用 BBS-GO，这是一个基于Go语言开发的社区系统。在开始安装前，请确保您已经准备好以下内容：",
        requirements: {
          title: "安装要求",
          mysql:
            "MySQL（5.7+）、PostgreSQL 或 SQLite 数据库（生产环境建议使用 MySQL 或 PostgreSQL）",
          site: "已经规划好社区名称和描述",
          admin: "已准备好管理员账号和密码",
        },
      },
      language: {
        title: "语言",
        description: "请选择安装和默认内容语言。",
        english: "English",
        chinese: "简体中文",
      },
      database: {
        description: "配置数据库连接信息，安装程序会先测试连接。",
        type: "数据库类型",
        host: "主机",
        hostPlaceholder: "localhost",
        hostHelp: "数据库服务器地址",
        port: "端口",
        portPlaceholder: "3306",
        name: "数据库名",
        namePlaceholder: "bbsgo",
        username: "用户名",
        usernamePlaceholder: "root",
        password: "密码",
        passwordPlaceholder: "数据库密码",
        sqliteWarning:
          "SQLite 适合本地测试或小规模部署，生产环境建议使用 MySQL 或 PostgreSQL。",
        sqliteAutoPath: "SQLite 数据库文件路径将由服务端自动处理。",
        dockerBuiltinMysqlNotice:
          "当前使用 Docker 内置 MySQL 部署，数据库连接由系统自动配置，安装时只需继续下一步。",
        validationError: "请完整填写数据库连接信息。",
        testing: "测试中...",
        testConnection: "测试连接",
        connectSuccess: "数据库连接成功",
        dockerBuiltinMysqlConnectSuccess: "Docker 内置 MySQL 连接成功",
        connectFailed: "数据库连接失败",
      },
      site: {
        description: "配置社区的基础信息。",
        title: "站点名称",
        titlePlaceholder: "请输入站点名称",
        siteDescription: "站点描述",
        descriptionPlaceholder: "请输入站点描述",
        baseURL: "站点地址",
        baseURLPlaceholder: "https://example.com",
        validationError: "请输入站点名称。",
        baseURLValidationError: "请输入站点地址。",
      },
      admin: {
        description: "创建第一个管理员账号。",
        username: "管理员用户名",
        usernamePlaceholder: "请输入管理员用户名",
        password: "管理员密码",
        passwordPlaceholder: "请输入管理员密码",
        confirmPassword: "确认密码",
        confirmPasswordPlaceholder: "请再次输入管理员密码",
        avatar: "管理员头像",
        randomAvatar: "随机头像",
        avatarDescription:
          "可随机选择一个默认头像，安装后也可以在个人资料中修改。",
        usernameError: "请输入管理员用户名。",
        passwordError: "请输入管理员密码。",
        confirmPasswordError: "请确认管理员密码。",
        passwordMismatchError: "两次输入的密码不一致。",
      },
      install: {
        preparing: "准备安装...",
        installing: "安装中...",
        connecting: "正在连接数据库并初始化数据...",
        completed: "安装完成",
        failed: "安装失败",
        unknown: "未知错误",
        requestFailed: "安装请求失败",
        description: "正在安装 BBS-GO，请不要关闭页面。",
      },
      complete: {
        congratulations: "恭喜，BBS-GO 已安装完成。",
        description: "现在可以进入站点或后台继续配置社区。",
        enterSite: "进入站点",
        enterAdmin: "进入后台",
      },
      buttons: {
        next: "下一步",
        previous: "上一步",
        install: "安装",
      },
    },
    user: {
      profile: "个人主页",
      personalCenter: "个人中心",
      topics: "话题",
      fans: "粉丝",
      followed: "关注",
    },
  },
  user: {
    signin: {
      title: "登录",
      callbackTitle: "登录中...",
      weixinLogin: "微信登录",
      weixinLoggingIn: "微信授权登录中...",
      hideWeixinQR: "收起二维码",
      passwordLogin: "账号登录",
      googleLogin: "Google 登录",
      googleLoginButton: "使用 Google 账号登录",
      googleLoggingIn: "登录中...",
      thirdPartyLogin: "或使用以下方式登录",
      githubLogin: "GitHub 登录",
      githubLoginButton: "使用 GitHub 账号登录",
      githubLoggingIn: "登录中...",
      missingAuthParams: "缺少必要的授权参数，请重新登录",
      password: {
        usernamePlaceholder: "请输入用户名",
        passwordPlaceholder: "请输入密码",
        loginBtn: "登录",
        usernameRequired: "请输入用户名",
        passwordRequired: "请输入密码",
      },
    },
    signup: {
      title: "注册账号",
      adminOnly: "账号仅可由管理员创建，请联系管理员。",
    },
    favorites: {
      title: "收藏列表",
      contentExpired: "收藏内容失效",
    },
    messages: {
      title: "消息",
      viewDetails: "查看详情",
    },
    passwordReset: {
      adminOnly: "密码不能自行找回或修改，请联系管理员处理。",
      forgot: {
        title: "找回密码",
      },
      reset: {
        title: "重置密码",
      },
    },
    profile: {
      title: "个人资料",
      backToProfile: "返回个人主页",
      avatar: "头像",
      nickname: "昵称",
      nicknamePlaceholder: "请输入昵称",
      username: "用户名",
      signature: "个性签名",
      signaturePlaceholder: "一句话介绍你自己",
      homepage: "个人主页",
      homepagePlaceholder: "请输入个人主页",
      editPlaceholder: "当前 MVP 暂未开放资料编辑。",
      save: "保存",
      editSuccess: "资料修改成功",
      editFailed: "资料修改失败",
      account: {
        title: "账号设置",
        username: "用户名",
        password: "密码",
        wechat: "微信",
        google: "Google",
        github: "GitHub",
        set: "设置",
        bind: "绑定",
        unbind: "解绑",
        passwordSet: "已设置",
        passwordNotSet: "未设置",
        wechatNotBound: "未绑定",
        googleNotBound: "未绑定",
        githubNotBound: "未绑定",
        unbindSuccess: "解绑成功",
        unbindFailed: "解绑失败",
        confirmUnbind: "确定要解除绑定？",
      },
    },
  },
  assigned: {
    title: "我认领的",
    empty: "你还没有认领任何提问。",
    pending: "待处理",
    solved: "已解决",
    dismissed: "已忽略",
    statusActive: "等待你的回复",
    statusResolved: "已解决",
    statusDismissed: "已被驳回",
    filterAll: "全部",
    progress: "已解决 {resolved}/{total}",
  },
} as const

export default zhCN
