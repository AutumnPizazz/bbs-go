...
import { XIcon, HandshakeIcon, Loader2Icon } from "lucide-react"
...
export function TopicClaimPanel({ topic }: { topic: Topic }) {
  ...
  const myClaim = claims.find((c) => Number(c.userId) === Number(currentUser.id) && c.status === "active")
  ...
  return (
    <div className="rounded-lg border bg-card p-4">
      ...
      {/* Claim button for non-author, non-admin users who haven't claimed */}
      {!isAuthor && !isAdmin && (
        hasClaimed ? (
          <Button
            variant="outline"
      ...
      {activeClaims.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-2">
          {activeClaims.map((c) => (
            ...
            >
              {t("common.user")}#{c.userId}
              {Number(c.userId) === Number(currentUser.id) ? ` (${t("component.claim.you")})` : ""}
              {/* Author/admin can dismiss, or self can unclaim */}
